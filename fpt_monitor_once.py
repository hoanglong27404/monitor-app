...
def analyze_with_claude(current, history):
    ...
    prompt = f"""Ban la chuyen gia phan tich chung khoan Viet Nam. Phan tich FPT:
    Gia: {fmt(current['price'])} VND | Thay doi: {current['change']:+,.0f} ({current['pct']:+.2f}%)
    Tham chieu: {fmt(current['ref'])} | Tran: {fmt(current['ceil'])} | San: {fmt(current['floor'])}
    Lich su 10 ngay:
    {hist}
